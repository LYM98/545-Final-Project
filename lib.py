...
def main(feature_directory,tag_directory, weight=False, iteration =20):
    '''
    ...
    Y_train, Y_valid = Y_train[:,:int(n*0.75)], Y_train[:,int(n*0.75):]
    ...
    if weight == True:
        W_new, B_new, total_loss = opt(W, B, X_train, Y_train, p, lamda, gamma, iteration)
    else:
...
def random_project(data,eps):
    a=np.random.RandomState(40)
    transformer=random_projection.GaussianRandomProjection(random_state=a, eps = eps)
...
def opt_periter(W=None, B=None, X=None, Y=None, P=None, Q=None, lamda=0.1, gamma=0.1):
    ...
    # print(Q[:10,:10])
    # print(temp.shape)
    B_new = (gamma*P+W_new@X@Y.T)@np.linalg.pinv(temp) # change here
...
def opt(W=None, B=None, X=None, Y=None, p=0.5, lamda=0.1, gamma=0.1, iteration = 15):
    '''
    ...
    while abs(loss-loss_list[-1])/loss_list[-1] > 0.0001 and iter_num < iteration: # stop iteration when loss decreases no larger than 5% or times of iteration over 20
...
def weight_opt(W=None, B=None, X=None, Y=None, p=0.5, lamda=0.1, gamma=0.1):
    ...
    while abs(loss-loss_list[-1])/loss_list[-1] > 0.05 and iter_num < 19: # stop iteration when loss decreases no larger than 5% or times of iteration over 20
        ...
        loss = total_loss_weighted(W_new, B_new, X, Y, P, Q, lamda, gamma, weight)
        iter_num += 1
        # print('Iteration: ',iter_num, 'Loss', loss)
    loss_list.append(loss)
...
if __name__ == "__main__":
    feature_directory = 'matlab_order_tags/combined_feature_all.npy'
    tag_directory= 'matlab_order_tags/binary_feature.npy'
    precision_list, recall_list, f1_score_list, non_zero_recall = main(feature_directory,tag_directory,True, 3)
    print(sum(precision_list) / len(precision_list), sum(recall_list) / len(recall_list),sum(f1_score_list) / len(f1_score_list), non_zero_recall)
